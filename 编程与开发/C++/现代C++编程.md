...
| ------ | ---------- | ---------------- | ------------------- | -------- | ---------------- |
| 1      | `[]`       | 数组下标索引     | array[index]        | 左到右   |                  |
|        | `()`       | 圆括号           | (expr)或func(para)  | 左到右   |                  |
|        | `.`        | 成员选择（对象） | obj.member          | 左到右   |                  |
|        | `->`       | 成员选择（指针） | obj_ptr->member     | 左到右   |                  |
| 2      | `-`        | 取负号           | -expr               | 右到左   | 单目             |
|        | `~`        | 按位取反         | ~expr               | 右到左   | 单目             |
|        | `!`        | 逻辑非           | !epxr               | 右到左   | 单目             |
|        | `++`       | 自增             | var++或++var        | 右到左   | 单目             |
|        | `--`       | 自减             | var--或--var        | 右到左   | 单目             |
|        | `&`        | 取地址           | &var                | 右到左   | 单目             |
|        | `*`        | 取指针所指的值   | *var_ptr            | 右到左   | 单目             |
|        | `new`      | 创建对象         | new objs[size]      | 右到左   | 单目             |
|        | `delete`   | 销毁对象         | delete objs         | 右到左   | 单目             |
|        | `(type)`   | 强制类型转换     | (type)epxr          | 右到左   |                  |
|        | `sizeof()` | 求所占字节数量   | sizeof(expr)        | 右到左   | 变量类型的字节数 |
| 3      | `*`        | 乘               | epxr * expr         | 左到右   | 双目             |
|        | `/`        | 除               | expr / expr         | 左到右   | 双目             |
|        | `%`        | 取模（余数）     | int_expr % int_epxr | 左到右   | 双目             |
| 4      | `+`        | 加               | epxr + expr         | 左到右   | 双目             |
|        | `-`        | 减               | expr - expr         | 左到右   | 双目             |
| 5      | `<<`       | 左移             | var << expr         | 左到右   | 双目             |
|        | `>>`       | 右移             | var >> expr         | 左到右   | 双目             |
| 6      | `>`        | 大于             | expr > expr         | 左到右   | 双目             |
|        | `>=`       | 大于等于         | expr >= expr        | 左到右   | 双目             |
|        | `<`        | 小于             | expr < expr         | 左到右   | 双目             |
|        | `<=`       | 小于等于         | expr <= expr        | 左到右   | 双目             |
| 7      | `==`       | 等于             | expr == expr        | 左到右   | 双目             |
|        | `!=`       | 不等于           | expr != expr        | 左到右   | 双目             |
| 8      | `&`        | 按位与           | expr & expr         | 左到右   | 双目             |
| 9      | `^`        | 按位异或         | expr ^ expr         | 左到右   | 双目             |
| 10     | `|`        | 按位或           | expr \| expr        | 左到右   | 双目             |
| 11     | `&&`       | 逻辑与           | expr && expr        | 左到右   | 双目             |
| 12     | `||`       | 逻辑或           | expr \|\| expr      | 左到右   | 双目             |
| 13     | `? :`      | 条件运算符       | expr ? expr : expr  | 右到左   | 三目             |
| 14     | `=`        | 赋值             | var = expr          | 右到左   |                  |
|        | `+=`       | 加后赋值         | var += expr         | 右到左   |                  |
|        | `-=`       | 减后赋值         | var -= expr         | 右到左   |                  |
|        | `*=`       | 乘后赋值         | var *= expr         | 右到左   |                  |
...
由于布局未定义，因此无法将这种类型传递到使用其他语言（例如C语言）编写的程序，并且由于它们可能是非连续（non-contiguous）的，因此无法使用快速低级函数（如memcopy函数）对其进行可靠的拷贝，或者通过网络对其进行序列化（serialize）。
...
- 在类中声明数据成员时，使用static关键字可以使类的所有实例共享该数据成员的副本。注意，类的static数据成员必须在类范围外文件范围内定义，声明为static const的整型数据成员可以有初始值。
