...
在Fermi架构中，一个SM有16个加载/存储单元（LD/ST Unit），一个时钟内可以为16个线程计算源地址和目标地址，并支持将每个地址的数据（一个字节）加载和存储到缓存或DRAM中。每个SM都有64KB的片上高速存储器，可配置为48KB的共享内存和16KB的L1缓存，或者，也可配置为16KB的共享内存和48KB的L1缓存。
...
NVIDIA并未公开太过详细的关于内存体系的描述，这里是针对GPU架构当中的MIO（Memory Input Output）队列与LD/ST（Load/Store）单元的硬件机制的猜测。一个SM通常划分为2或4个处理块分区，一个处理块分区通常1个MIO队列，以及4或8个LD/ST单元。

在一个Warp调度执行时，会将所有与内存相关的指令发射到该Warp所在的SM处理块分区上的MIO队列中等待执行，包括设备全局内存指令，共享内存指令等。如果MIO队列已满，则当前时钟内无法为Warp调度内存相关指令。

这些内存指令会由LD/ST单元负责异步执行，一个LD/ST单元可以在一个时钟内为一个线程计算源地址和目标地址，并支持将每个地址的数据（一个字节）加载和存储到缓存或DRAM中。实际上，LD/ST单元会将任务分配给下游硬件，例如共享内存指令的下游硬件是共享内存控制器，全局内存指令的下游硬件是全局内存控制器等。如果下游硬件在忙，则当前周期内LD/ST无法执行新的内存指令。

之后，由相应的内存控制器将数据从物理存储体中读取给LD单元，或将来自ST单元的数据写入到物理存储体当中。

对于共享内存而言，若将共享内存数据加载到寄存器或将寄存器数据存储到共享内存时，一个LD/ST单元负责计算一个地址，而共享内存的32个bank采用4字节位宽的交叉编址，于是一个LD/ST单元可以负责共享内存的4字节数据。于是，若一个SM拥有16个LD/ST单元，则其共享内存带宽为64Byte/cycle，若一个SM拥有32个LD/ST单元，则其共享内存带宽为128Byte/cycle。
