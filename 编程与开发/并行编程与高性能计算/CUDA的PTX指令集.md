...
张量映射（Tensor Map）是一个位数为128字节的不透明结构体对象，位于.const常量存储状态空间或者.param内核参数存储状态空间或者.global全局存储状态空间中，该不透明的对象描述了一个张量的张量属性和访问属性，这些属性将在本节介绍。可以使用CUDA编程环境中的API函数创建一个张量映射对象。
