...
PTX并没有公开堆栈布局、函数调用约定、程序二进制接口ABI的详细信息，而是提供了更高级别的ABI抽象，以支持多个ABI实现。本节将介绍PTX所提供的ABI抽象的功能，包括函数定义、函数调用、参数传递、可变函数参数的支持、堆栈上的内存分配。

在PTX中，使用.func指令声明和定义一个函数，函数声明指定函数的名称、可选的输入参数列表、可选的返回值列表，函数定义指定函数的主体。函数在调用之前必须声明或定义。标量参数和向量参数可以使用寄存器变量传递，返回值可以直接放入到寄存器变量中，数据类型和大小必须相匹配。在使用抽象ABI时，作为参数的.reg变量的大小必须至少为32位，在C++/PTX混合编程环境中，在C++中不足32位的数据，在PTX中应该提升为32位寄存器。

使用call指令调用一个函数，将控制权转移给函数，并隐式保存返回地址，在函数体的最后使用ret指令将控制权转移给调用者的下一条指令。如下所示。

```
.func (.reg.u32 %res) func_inc_ptr (.reg.u32 %ptr, .reg.u32 %inc) {
    add.u32 %res, %ptr, %inc;
    ret;
}
